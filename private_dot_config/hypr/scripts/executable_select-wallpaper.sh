...
# Get primary/focused monitor for rofi display calculations
primary_monitor=$(hyprctl monitors -j | jq -r '.[] | select(.focused) | .name')

# If no focused monitor, use the first available monitor
if [[ -z "$primary_monitor" ]]; then
  primary_monitor=$(hyprctl monitors -j | jq -r '.[0].name')
fi

# Ensure at least one monitor is detected
if [[ -z "$primary_monitor" ]]; then
  notify-send -i "$iDIR/error.png" "E-R-R-O-R" "Could not detect any monitors"
  exit 1
fi

# Monitor details for rofi icon sizing (using primary monitor)
scale_factor=$(hyprctl monitors -j | jq -r --arg mon "$primary_monitor" '.[] | select(.name == $mon) | .scale')
monitor_height=$(hyprctl monitors -j | jq -r --arg mon "$primary_monitor" '.[] | select(.name == $mon) | .height')
...
modify_startup_config() {
  ...
  local startup_config="$HOME/.config/hypr/user/startup.conf"
...
apply_image_wallpaper() {
  ...
  for monitor in "${monitors[@]}"; do
    echo "Applying wallpaper to monitor: $monitor"
