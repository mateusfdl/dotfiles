...
Singleton {
    ...
    readonly property string defaultKeybindConfigPath: Files.trimFileProtocol(`${Directories.config}/hypr/configs/keybinds.conf`)
    readonly property string userKeybindConfigPath: Files.trimFileProtocol(`${Directories.config}/hypr/user/keybinds.conf`)
    ...
    function parseKeyName(raw) {
        ...
        if (root.codeMap[lower])
            return root.codeMap[lower];
        if (root.keyNameMap[lower])
            return root.keyNameMap[lower];
    ...
    function describeAction(dispatcher, args) {
        ...
        if (d === "exec") {
            ...
            if (a.includes("hyprctl reload"))
                return "Reload config";
            if (a.includes("hyprctl dispatch exit"))
                return "Exit Hyprland";
            if (a.includes("hyprctl dispatch splitratio"))
                return "Set split ratio";
            if (a.includes("hyprctl dispatch workspaceopt allfloat"))
                return "Toggle all float";
            if (a.includes("hyprctl setprop active opaque"))
                return "Toggle opacity";
            if (a.includes("hyprctl keyword cursor:zoom_factor")) {
                return a.includes("* 2") ? "Zoom in" : "Zoom out";
            }
            if (a.includes("wpctl set-volume") && a.includes("+"))
                return "Volume up";
            if (a.includes("wpctl set-volume") && a.includes("-"))
                return "Volume down";
            if (a.includes("wpctl set-mute") && a.includes("SINK"))
                return "Toggle mute";
            if (a.includes("wpctl set-mute") && a.includes("SOURCE"))
                return "Toggle mic mute";
            if (a.includes("systemctl suspend"))
                return "Suspend";
            if (a.includes("screenshot.sh")) {
                ...
                if (a.includes("--in5"))
                    return "Screenshot (5s delay)";
                if (a.includes("--in10"))
                    return "Screenshot (10s delay)";
                if (a.includes("--active"))
                    return "Screenshot (window)";
                return "Screenshot";
            }
            if (a.includes("xdg-open"))
                return "Open browser";
        ...
        if (d === "global") {
            if (a.includes("launcherToggle"))
                return "Toggle launcher";
            if (a.includes("overviewToggle"))
                return "Toggle overview";
            if (a.includes("cheatsheetToggle"))
                return "Toggle cheatsheet";
            if (a.includes("windowSwitcherNext"))
                return "Next window";
            if (a.includes("windowSwitcherPrevious"))
                return "Previous window";
            if (a.includes("windowSwitcherActivate"))
        ...
        if (d === "workspace" || d === "movetoworkspace" || d === "movetoworkspacesilent") {
            ...
            if (a.startsWith("m-"))
                return name + " (prev)";
            if (a.startsWith("e+"))
                return name + " (scroll next)";
            if (a.startsWith("e-"))
                return name + " (scroll prev)";
            if (a === "-1" || a === "+1")
                return name + " " + a;
            return name + " " + a;
        }
    ...
    function parseLine(line) {
        ...
        if (flags.includes("m") && !flags.includes("e"))
            return null;
    ...
    function rebuildKeybinds() {
        ...
        console.log("[HyprlandKeybinds] Parsed", sections.length, "sections,", sections.reduce((acc, s) => acc + s.binds.length, 0), "total binds");
