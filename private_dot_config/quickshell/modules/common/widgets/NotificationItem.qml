...
Item { // Notification item area
    ...
    Rectangle { // Background of notification item
        ...
        color: (expanded && !onlyNotification) ? (notificationObject.urgency == NotificationUrgency.Critical) ? Colors.mix(Appearance.colors.colSecondaryContainer, Appearance.colors.colLayer2, 0.35) : (Appearance.colors.colLayer3) : Colors.transparentize(Appearance.colors.colLayer3)
        ...
        ColumnLayout { // Content column
            ...
            ColumnLayout {
                ...
                StyledText { // Notification body (expanded)
                    ...
                    text: {
                        return `<style>img{max-width:${300 /* binding to notificationBodyText.width would cause a binding loop */}px;}</style>` + `${processNotificationBody(notificationObject.body, notificationObject.appName || notificationObject.summary).replace(/\n/g, "<br/>")}`;
                    }
