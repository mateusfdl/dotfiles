            ...
            Item {
                ...
                Keys.onPressed: event => {
                    if (event.key === Qt.Key_Escape) {
                        ...
                        var nextId = HyprlandWorkspace.navigateForward(root.monitorWorkspaceIds, root.monitor.activeWorkspace?.id ?? -1);
                        if (nextId >= 0)
                            Hyprland.dispatch(`workspace ${root.workspaceNameForId(nextId)}`);
                        event.accepted = true;
                    } else if (event.key === Qt.Key_B || event.key === Qt.Key_H || event.key === Qt.Key_Left) {
                        var prevId = HyprlandWorkspace.navigateBackward(root.monitorWorkspaceIds, root.monitor.activeWorkspace?.id ?? -1);
                        if (prevId >= 0)
                            Hyprland.dispatch(`workspace ${root.workspaceNameForId(prevId)}`);
                        event.accepted = true;
                    } else if (event.key === Qt.Key_N) {
                        var newId = HyprlandWorkspace.insertWorkspaceAfter(root.monitorWorkspaceIds, root.monitor.activeWorkspace?.id ?? -1);
                        if (newId >= 0)
                            Hyprland.dispatch(`workspace ${newId}`);
                        event.accepted = true;
                    }
                }
