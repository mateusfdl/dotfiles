...
static void pwOnStreamStateChanged(void * /*userdata*/,
                                   enum pw_stream_state /*old*/,
                                   enum pw_stream_state state,
                                   const char *error) {
  switch (state) {
  case PW_STREAM_STATE_ERROR:
    qWarning() << "[AudioSpectrum] Stream error:"
               << (error ? error : "unknown");
    break;
...
AudioSpectrum::AudioSpectrum(QObject *parent)
    : QObject(parent), m_ringBuffer(BUFFER_SIZE, 0.0f),
      m_magnitudes(SPECTRUM_SIZE, 0.0), m_smoothedBars(m_barCount, 0.0) {
...
void AudioSpectrum::precomputeBandBins() {
  ...
  for (int band = 0; band < m_barCount; ++band) {
    const double freqLow =
        std::pow(10.0, logMin + logRange * band / m_barCount);
    const double freqHigh =
        std::pow(10.0, logMin + logRange * (band + 1) / m_barCount);

    int binLow = static_cast<int>(std::floor(freqLow * FFT_SIZE / kSampleRate));
    int binHigh =
        static_cast<int>(std::ceil(freqHigh * FFT_SIZE / kSampleRate));
...
void AudioSpectrum::pipewireThreadFunc() {
  ...
  pw_properties *props = pw_properties_new(
      ...
      PW_KEY_MEDIA_ROLE, "DSP", PW_KEY_STREAM_CAPTURE_SINK, "true",
      PW_KEY_NODE_NAME, "quickshell-spectrum", PW_KEY_APP_NAME,
      "QuickShell Spectrum", nullptr);
...
void AudioSpectrum::processFFT() {
  ...
  for (int i = 0; i < SPECTRUM_SIZE; ++i) {
    ...
    m_magnitudes[static_cast<size_t>(i)] =
  ...
  for (int band = 0; band < m_barCount; ++band) {
    ...
    val = (val > kMinMagnitude)
              ? (std::log10(val * kLogBase + 1.0) / kLogDivisor)
              : 0.0;
  ...
  for (int i = 0; i < m_barCount; ++i) {
    ...
    const double alpha =
        (target > current) ? (1.0 - m_smoothing * 0.5) : (1.0 - m_smoothing);

    double smoothed = current + alpha * (target - current);

    if (smoothed < kMinBarThreshold)
      smoothed = 0.0;
