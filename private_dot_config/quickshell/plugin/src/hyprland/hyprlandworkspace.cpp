...
QList<int> sortedCopy(const QList<int> &ids) {
  auto sorted = ids;
...
int HyprlandWorkspace::navigateForward(const QList<int> &workspaceIds,
                                       int currentId) {
  if (workspaceIds.isEmpty())
    return -1;

  const auto sorted = sortedCopy(workspaceIds);
  const auto it = std::find(sorted.cbegin(), sorted.cend(), currentId);

  if (it == sorted.cend() || std::next(it) == sorted.cend())
    return -1;

  return *std::next(it);
}

int HyprlandWorkspace::navigateBackward(const QList<int> &workspaceIds,
                                        int currentId) {
  if (workspaceIds.isEmpty())
    return -1;
  ...
  if (it == sorted.cend() || it == sorted.cbegin())
    return -1;

  return *std::prev(it);
}

int HyprlandWorkspace::insertWorkspaceAfter(const QList<int> &workspaceIds,
                                            int currentId) {
  if (currentId < 0)
    return -1;
  ...
  for (auto it = sorted.crbegin(); it != sorted.crend(); ++it) {
    if (*it >= newSlot) {
      const auto cmd =
          QStringLiteral("renameworkspace %1 %2").arg(*it).arg(*it + 1);
...
QString HyprlandWorkspace::hyprlandSocketPath() {
  ...
  if (!xdgRuntime.isEmpty()) {
    const auto path =
        QStringLiteral("%1/hypr/%2/.socket.sock").arg(xdgRuntime, sig);
...
bool HyprlandWorkspace::hyprlandDispatch(const QString &command) {
  ...
  UniqueFd sock(::socket(AF_UNIX, SOCK_STREAM, 0));
  if (!sock)
    return false;
  ...
  std::memcpy(addr.sun_path, pathBytes.constData(),
  ...
  const auto written = ::write(sock.fd, payload.constData(),
  ...
  const auto bytesRead = ::read(sock.fd, buf.data(), buf.size() - 1);

  if (bytesRead <= 0)
    return false;

  const auto response =
      QString::fromUtf8(buf.data(), static_cast<int>(bytesRead)).trimmed();
