...
QColor Colors::mix(const QColor& color1, const QColor& color2, qreal ratio) {
    ...
    return QColor::fromRgbF(
        ratio * color1.redF()   + inv * color2.redF(),
...
QColor Colors::colorWithSaturationOf(const QColor& color1, const QColor& color2) {
    float h1, v1, a1, s2;
...
QColor Colors::colorWithLightnessOf(const QColor& color1, const QColor& color2) {
    float l2;
...
qreal Colors::luminance(const QColor& color) {
    return std::sqrt(
        0.299 * color.redF()   * color.redF()   +
        0.587 * color.greenF() * color.greenF() +
        0.114 * color.blueF()  * color.blueF()
    );
}

bool Colors::isDark(const QColor& color) {
