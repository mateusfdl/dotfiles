...
public:
    ...
    Q_INVOKABLE static QVariantList go(const QString& search, const QVariantList& targets,
...
private:
    ...
    static MatchResult algorithm(const PreparedSearch& search, PreparedTarget& target,
                                 bool allowSpaces = false, bool allowPartialMatch = false);
    static MatchResult algorithmSpaces(const PreparedSearch& search, PreparedTarget& target,
                                       bool allowPartialMatch);
    static double normalizeScore(double score);

    static PreparedTarget variantToPrepared(const QVariantMap& map);
    static QVariantList allResults(const QVariantList& targets, const QVariantMap& options);
};
