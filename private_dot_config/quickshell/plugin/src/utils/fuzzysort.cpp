...
std::vector<int> FuzzySort::prepareBeginningIndexes(const QString& target) {
    ...
    for (int i = 0; i < targetLen; ++i) {
        ...
        const bool isAlphanum = isUpper || (code >= 97 && code <= 122) || (code >= 48 && code <= 57);
        const bool isBeginning = (isUpper && !wasUpper) || !wasAlphanum || !isAlphanum;
        wasUpper = isUpper;
        wasAlphanum = isAlphanum;
        if (isBeginning) beginningIndexes.push_back(i);
    }
...
std::vector<int> FuzzySort::prepareNextBeginningIndexes(const QString& target) {
    ...
    int lastIsBeginning = beginningIndexes.empty() ? targetLen : beginningIndexes[0];
...
FuzzySort::PreparedSearch FuzzySort::prepareSearchParsed(const QString& search) {
    ...
    if (info.containsSpace) {
        ...
        for (const auto& part : parts) {
            if (part.isEmpty() || seen.contains(part)) continue;
            seen.insert(part);
...
double FuzzySort::normalizeScore(double score) {
    if (score == NEG_INF) return 0.0;
    if (score > 1.0) return score;
    return std::exp((std::pow(-score + 1.0, 0.04307) - 1.0) * -2.0);
}

FuzzySort::MatchResult FuzzySort::algorithm(
    const PreparedSearch& preparedSearch,
    ...
    matchesSimple.reserve(searchLen);
    {
        ...
        if (searchI != searchLen) return {};
    }

    if (!prepared.nextBeginningIndexesReady) {
        prepared.nextBeginningIndexes = prepareNextBeginningIndexes(prepared.target);
    ...
    {
        int searchI = 0;
        int targetI = (matchesSimple[0] == 0) ? 0 : nextBeginningIndexes[matchesSimple[0] - 1];
    ...
    const int substringIndex = (searchLen <= 1) ? -1 : prepared.targetLower.indexOf(preparedSearch.lower, matchesSimple[0]);
    bool isSubstring = (substringIndex != -1);
    bool isSubstringBeginning = false;
    if (isSubstring) {
        isSubstringBeginning = (substringIndex == 0) ||
            (nextBeginningIndexes[substringIndex - 1] == substringIndex);
    }

    int finalSubstringIndex = substringIndex;
    if (isSubstring && !isSubstringBeginning) {
        for (int i = 0; i < static_cast<int>(nextBeginningIndexes.size()); i = nextBeginningIndexes[i]) {
            if (i <= substringIndex) continue;
            bool matched = true;
            for (int s = 0; s < searchLen; ++s) {
                if (i + s >= targetLen || searchLowerCodes[s] != targetLowerCodes[i + s]) {
                    matched = false;
                    break;
                }
            }
    ...
    auto calculateScore = [&](const std::vector<int>& matches) -> double {
        ...
        const int unmatchedDistance = matches[searchLen - 1] - matches[0] - (searchLen - 1);
        ...
        if (!successStrict) {
            ...
            for (int i = nextBeginningIndexes[0]; i < targetLen; i = nextBeginningIndexes[i]) {
    ...
    if (!successStrict) {
        if (isSubstring) {
            for (int i = 0; i < searchLen; ++i) matchesSimple[i] = finalSubstringIndex + i;
        }
...
FuzzySort::MatchResult FuzzySort::algorithmSpaces(
    const PreparedSearch& preparedSearch,
    ...
    for (int i = 0; i < searchesLen; ++i) {
        ...
        if (!isTheLastSearch && result.valid) {
            ...
            if (consecutive && !indexes.empty()) {
                ...
                if (newBeginningIndex > 0 && newBeginningIndex - 1 < static_cast<int>(target.nextBeginningIndexes.size())) {
                    const int toReplace = target.nextBeginningIndexes[newBeginningIndex - 1];
        ...
        if (result.valid) {
            ...
            if (!result.indexes.empty() && result.indexes[0] < firstSeenIndexLastSearch) {
    ...
    seenIndexes.erase(std::unique(seenIndexes.begin(), seenIndexes.end()), seenIndexes.end());
...
QVariantList FuzzySort::allResults(const QVariantList& targets, const QVariantMap& options) {
    ...
    if (!key.isEmpty()) {
        for (const auto& item : targets) {
            ...
            entry[QStringLiteral("target")] = targetMap.value(QStringLiteral("target"));
        ...
        for (const auto& item : targets) {
            ...
            if (item.typeId() == QMetaType::QVariantMap) {
                entry[QStringLiteral("target")] = item.toMap().value(QStringLiteral("target"));
            } else {
...
QVariantList FuzzySort::go(const QString& search, const QVariantList& targets,
    ...
    if (!key.isEmpty()) {
        for (const auto& item : targets) {
            ...
            if (!targetVal.isValid()) continue;

            const QVariantMap targetMap = targetVal.toMap();
            if (targetMap.isEmpty()) continue;

            const int targetBitflags = targetMap.value(QStringLiteral("_bitflags")).toInt();
            if ((searchBitflags & targetBitflags) != searchBitflags) continue;
        ...
        for (const auto& item : targets) {
            ...
            if (item.typeId() == QMetaType::QVariantMap) {
                ...
                if (map.contains(QStringLiteral("_prepared"))) {
                    ...
                    prepared = prepareParsed(map.value(QStringLiteral("target")).toString());
                }
            } else {
