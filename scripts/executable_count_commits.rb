...
if ARGV.include?('-h') || ARGV.include?('--help') then puts <<~HELP
...
def get_dirs(dirs)
  ...
  dirs = Dir.glob('*/') if dirs.empty?
  # Remove non-dirs
  dirs.select! { |d| File.directory? d }
  # Remove trailing slashes
  dirs.map! { |d| d.sub %r{/$}, '' }
  # Fail if no dirs were found.
  fatal 'Error: No directories found.' if dirs.empty?
  dirs
end
...
def add_commit(email, name)
  ...
  if committer_by_email && committer_by_name && (committer_by_email != committer_by_name)
...
get_dirs(ARGV).each do |dir|
  ...
  if !stderr.readlines.empty?
    warn %(Error running "git log" in "#{dir}" directory.)
  else
    $project_count += 1
...
$committers.sort! do |a, b|
  ...
  com.zero? ? (a[:names][0].downcase <=> b[:names][0].downcase) : comp
end

# Whoops?
$total = get_computed_total
fatal "Error: Parsed #{$commit_count} commits, but for some reason only #{$total} appear." if $total != $commit_count

# Useful output to STDOUT
$committers.each do |c|
  puts format("%#{$total.to_s.length}d %s (%s)", c[:commit_count], c[:emails].join(', '), c[:names].join(', '))
end
